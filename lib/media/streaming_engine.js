...
shaka.media.StreamingEngine = class {
  ...
  update_(mediaState) {
    ...
    // packagings where one reference is one frame — LOC over MoQ, where a
    // 48 kHz AAC reference is 21.3 ms — one segment of run-ahead is 21.3 ms,
    // so a stream is throttled the instant it appends anything, and pays
    // updateIntervalSeconds for it. Audio and video then ping-pong, each
    // appending a few objects and sleeping, which pins the COMBINED append
    // rate far below what either stream needs.
    //
    // Measured on a live LOC stream (JSS#687): combined appends sat at
    // 72-77/s across every configuration tried, against a demand of ~107/s.
    // Raising this allowance alone, changing nothing else, took combined
    // appends to 135/s and the audio buffer from 0.4 s to 5.8 s.
    //
    // The floor is in the time domain because that is what the rule is
    // actually about — how far apart the streams may drift — and it binds
    // only when segments are shorter than it, so DASH/HLS with
    // multi-second segments are unaffected.
...
/**
 * The minimum run-ahead allowance, in seconds, regardless of segment duration.
 *
