...
shaka.msf.TracksManager = class {
  ...
  async handleSubgroupStream_(reader, streamType) {
    try {
      ...
      while (!(await reader.done())) {
        ...
        // Read the Object ID Delta.  Objects in a subgroup are in ascending
        // Object ID order, so the wire carries the gap to the previous
        // object rather than an absolute ID: the first object in the
        // subgroup is absolute, and every subsequent object is
        // previous + delta + 1 (a delta of 0 means consecutive IDs).
        // eslint-disable-next-line no-await-in-loop
  ...
  async unsubscribeTrack(trackAlias) {
    ...
    try {
      ...
      // acknowledgement to wait on — and a re-subscribe can easily complete
      // inside it. The relay reuses the same trackAlias for the same track
      // name, so `unregisterAllCallbacks(trackAlias)` would then delete the
      // callbacks belonging to the NEW subscription, silently and permanently.
      //
      // That is exactly what killed audio (JSS#664): a video ABR switch
      // unsubscribes every track and immediately re-subscribes, so audio0 went
      //
      //   t+0.00  unsubscribe audio0 (requestId 4), start 0.5s timer
      //   t+0.01  subscribe   audio0 (requestId 6)
      //   t+0.04  SubscribeOk audio0 -> same trackAlias 10, callbacks live
      //   t+0.50  timer fires -> unregisterAllCallbacks(10) -> audio dead
      //
      // Video survived only because the switch moves to a different track
      // name (video0 -> video1) and therefore a different alias. Audio, which
      // never changes, was destroyed on the first quality switch every time.
      const current = this.trackRegistry_.getTrackInfoFromAlias(trackAlias);
      if (current && current.requestId !== requestId) {
        shaka.log.debug(
            `Track alias ${trackAlias} was re-subscribed (requestId ` +
            `${current.requestId}) while the unsubscribe of requestId ` +
            `${requestId} was in flight; leaving the new subscription intact`);
        return;
      }
