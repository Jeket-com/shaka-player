...
shaka.msf.MSFParser = class {
  ...
  processTrack_(track, contentProtections, initDataList) {
    ...
    // LOC tracks carry raw elementary-stream frames (AAC / Annex-B H.264), not
    // MP4, so they have no init segment on the wire — LocTransmuxer builds the
    // MP4 init itself via Mp4Generator (BaseTransmuxer.packageSegment returns
    // it as `init`). Synthesising an InitSegmentReference here made
    // MediaSourceEngine append it, and an init-segment append passes
    // reference=null into transmux(), which dereferences it unconditionally:
    //
    //   TypeError: Cannot read properties of null (reading 'startTime')
    //       at LocTransmuxer.getAacStreamInfo_
    //
    // That aborted every append. It surfaced only as `failed fetch and append:
    // code=undefined` and a permanently stalled player, because the thrown
    // value is a raw TypeError rather than a shaka.util.Error (JSS#643).
    ...
    if (isLoC) {
      ...
      locParser = new shaka.msf.LOCParser(frameDuration, type);
    }
    ...
    stream.createSegmentIndex = () => {
      ...
      this.subscribeToTrack(track, trackKey, (obj) => {
        ...
        if (timelineLocked) {
          // Retain references for SEGMENT_INDEX_RETENTION_SEC, not 2 s.
          //
          // StreamingEngine walks the index in order from its last appended
          // reference; when the one it wants next has been evicted,
          // getSegmentReferenceNeeded_ returns null and update_ parks in its
          // "segment could not be found" retry. It then falls further behind,
          // so the eviction that caused the stall guarantees the stall
          // persists — the engine can never catch up to content that is
          // deleted ahead of it.
          //
          // Measured on a live 48 kHz AAC stream (JSS#687): the index held
          // exactly 95 references (2.0 s) while the engine ran up to 10.6 s
          // behind the newest reference, with the iterator null on 45 of 59
          // samples. Widening retention to 12 s was the only change made and
          // audio appends went 13.3/s -> 34.6/s with readyState 2 -> 4.
          //
          // 2 s is only safe when a reference is a multi-second segment. Here
          // one reference is a single 21 ms frame, so 2 s is 95 of them and
          // any hiccup longer than that is unrecoverable. References are
          // metadata, so retention is cheap: 20 s of audio is ~940 of them.
          const evictTime = Math.min(
              reference.startTime -
                  shaka.msf.MSFParser.SEGMENT_INDEX_RETENTION_SEC,
        ...
        if (this.presentationTimeline_.isDynamic()) {
          ...
          if (maxSegmentDuration > 0) {
            // No DVR — the seek range should sit at the live edge. But the
            // window still has to be wide enough to CONTAIN the playhead.
            //
            // "availability window = one segment" is right when a segment is
            // seconds long. On the MSF/LOC path one object is one FRAME, so
            // maxSegmentDuration is ~21 ms for 48 kHz AAC and ~17 ms for 60 fps
            // video. That produced a 21 ms-wide seek range, and the playhead
            // — which necessarily sits a little behind the live edge, by
            // whatever is buffered — fell outside it immediately:
            //
            //   seekRange   = [1786214272.6686, 1786214272.6899]   (21 ms)
            //   currentTime =  1786214272.59                        (80 ms behind)
            //
            // shaka then re-seeks the playhead back into range, over and over:
            // 23 gapsJumped in 30 s, ~10% dropped frames, and an audio buffer
            // that could never grow past ~0.9 s.
            //
            // So floor the window at the buffer we are actually trying to hold.
            // This is not DVR: the range still tracks the live edge, it is just
            // wide enough for the playhead to live inside it. The floor matches
            // shaka's default streaming.bufferingGoal — the parser only gets
            // the manifest config, so it cannot read the streaming value
            // directly, but that is the quantity this needs to cover.
            const MIN_AVAILABILITY_SEC =
                shaka.msf.MSFParser.MIN_AVAILABILITY_WINDOW_SEC;
            this.presentationTimeline_.setSegmentAvailabilityDuration(
                Math.max(maxSegmentDuration, MIN_AVAILABILITY_SEC));
          }
        }
 ...
 *
 * The window has to contain the playhead, which necessarily trails the live
 * edge by however much is buffered. Deriving it from one object's duration
 * gives ~21 ms on this path — one AAC frame — and the playhead then falls
 * outside its own seek range and is re-seeked continuously.
 *
 ...
 * @const {number}
 */
shaka.msf.MSFParser.MIN_AVAILABILITY_WINDOW_SEC = 10;
 ...
 * evicted. On the LOC path one reference is one frame (21 ms for 48 kHz AAC),
 * so this is a large number of references but a small amount of memory.
 *
 * @const {number}
 */
shaka.msf.MSFParser.SEGMENT_INDEX_RETENTION_SEC = 20;
