...
shaka.msf.MSFParser = class {
  constructor() {
    ...
    /**
     * Presentation-time origin, in seconds, subtracted from every segment's
     * media timestamp.
     *
     * LOC Timestamps are wall-clock microseconds, so raw derived times are
     * epoch values (~1.785e9 s). Using those directly as presentation time
     * deadlocks playback: media lands at t≈1.785e9 while the video element's
     * playhead is at 0, so nothing appends at 0, readyState stays
     * HAVE_NOTHING, seekable stays empty, and the playhead is never moved to
     * the live edge. Nothing errors, so nothing is logged.
     *
     * Latching the first timestamp we see and subtracting it puts the
     * presentation near t=0. It is deliberately shared across every track —
     * audio and video must use the SAME origin or they desync by the
     * difference — and latched once for the life of the presentation, so the
     * timeline never jumps.
     *
     * @private {?number}
     */
    this.presentationTimeOrigin_ = null;
  }
  ...
  processTrack_(track, contentProtections, initDataList) {
    ...
    stream.createSegmentIndex = () => {
      ...
      this.subscribeToTrack(track, trackKey, (obj) => {
        ...
        // Rebase onto a small presentation-time origin. See
        // presentationTimeOrigin_ — latched once, shared by every track.
        //
        // The origin sits a fixed headroom BELOW the first timestamp we see.
        // Tracks do not start perfectly aligned, so whichever track arrives
        // first would otherwise force any earlier-timestamped track negative;
        // the headroom absorbs that skew and keeps presentation times small
        // and positive.
        if (this.presentationTimeOrigin_ === null) {
          const kOriginHeadroomSeconds = 10;
          this.presentationTimeOrigin_ = startTime - kOriginHeadroomSeconds;
          shaka.log.debug('MSF: presentation time origin latched at ' +
              this.presentationTimeOrigin_ + 's (first timestamp ' + startTime +
              's); presentation times are relative to it');
        }
        startTime -= this.presentationTimeOrigin_;
        if (startTime < 0) {
          // Cross-track skew larger than the headroom. Dropping is better than
          // appending at a negative presentation time, which MSE rejects.
          shaka.log.warning('MSF: dropping segment ' + startTime + 's before ' +
              'the presentation origin (track skew exceeds headroom)');
          return;
        }
