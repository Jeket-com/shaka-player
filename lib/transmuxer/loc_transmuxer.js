...
shaka.transmuxer.LocTransmuxer = class extends shaka.transmuxer.BaseTransmuxer {
  ...
  constructor(mimeType) {
    ...
    /**
     * `id` of the stream the cached configs above were parsed from. A single
     * transmuxer instance is reused across an ABR switch, so this is what
  ...
  transmux(data, stream, reference, duration, contentType) {
    ...
    // Drop cached decoder configs when the rendition changes. MediaSourceEngine
    // keeps one transmuxer per content type, and every LOC tier normalises to
    // the same base codec ('avc'), so an ABR switch takes the ResetMode.NONE
    // path: same SourceBuffer, same transmuxer, no changeType(). The cached
    // SPS/PPS would otherwise survive the switch, and because a switch
    // typically lands mid-GOP the first frames of the new tier are non-IDR and
    // cannot refresh it — so the init segment generated for the new stream
    // would describe the OLD tier's resolution and the decoder would fail on
    // the very first sample. Clearing here suppresses output until the new
    // tier's first IDR, which is the same behaviour as at startup.
  ...
  getAacStreamInfo_(data, stream, duration, reference) {
    ...
    // Strip the ADTS header if the producer left one on.
    //
    // An mp4a sample entry describes the stream through its esds
    // (AudioSpecificConfig), so samples must be RAW AAC access units. A frame
    // that still carries its 7- or 9-byte ADTS header is not a valid access
    // unit: the decoder is configured from the esds, then receives a packet
    // beginning with the 0xFFFx syncword and rejects it. Chromium reports
    // "Failed to send audio packet for decoding" on the very first packet and
    // fails the whole pipeline, taking video down with it.
    //
    // The LOC producer is specified to strip ADTS, but observed streams do
    // carry it (mdat payload beginning `ff f1 4c 80 ...`), so detect rather
    // than assume — the header is self-describing and this is a no-op on
    // conforming input.
    ...
    if (data.length > 2 && ADTS.isHeaderPattern(data, 0)) {
      ...
      if (header) {
        audioData = data.subarray(header.headerLength,
            header.headerLength + header.frameLength);
      }
    }
